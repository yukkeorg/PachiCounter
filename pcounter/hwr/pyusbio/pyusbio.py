...
class USBIO(object):
        ...
        for product_id in (PRODUCT_ID_ORIG, PRODUCT_ID_AKI):
            ...
            if self._device:
                ...
                if product_id == PRODUCT_ID_ORIG:
                    ...
                    raise PyUSBIOError("Unknown PRODUCT_ID: "
                                       "0x{0:x}".format(product_id))

                logger.info("USB-IO(0x{0:x}, 0x{1:x}) "
                            "is found.".format(VENDOR_ID, product_id))
                break
        if not self._device:
            ...
            for ifc in cnf:
                for ep in ifc:
                    if ep.bEndpointAddress & usb.util._ENDPOINT_DIR_MASK:
                        ...
                        logger.debug("IN endpoint address of interface {0} : "
                                     "0x{1:x}".format(ifc.bInterfaceNumber,
                                                      ep.bEndpointAddress))
                    else:
                        self._outEpAddr = ep.bEndpointAddress
                        logger.debug("OUT endpoint address of interface {0} : "
                                     "0x{1:x}".format(ifc.bInterfaceNumber,
                                                      ep.bEndpointAddress))

                if self._device.is_kernel_driver_active(ifc.bInterfaceNumber):
                    self._device.detach_kernel_driver(ifc.bInterfaceNumber)
                    logger.info("interface {0} of USB-IO is detached from "
                                "the kernel driver.".format(ifc.bInterfaceNumber))
        return True
    ...
    def _cmd(self, command, writedata=None, do_read=True):
        ...
        if do_read:
            data = self._device.read(self._inEpAddr, self._cmdsize,
                                     timeout=self._timeout)
            if (data[0] != cmd[0] or
                    data[self._cmdsize-1] != cmd[self._cmdsize-1]):
    ...
    def setSysConf(self, sysconf):
        ...
        return self._cmd(CMD_SYSCONF_WRITE,
                         writedata=sysconf.toArray(), do_read=False)


class SysConf(object):
    def __init__(self, is_pullup=True,
                 port1=0x00, port2=0x0f, init1=0x00, init2=0x00):
    ...
    def copy(self):
        return SysConf(self.is_pullup,
                       self.port1, self.port2,
                       self.init1, self.init2)

    def __repr__(self):
        return ("<SysConf is_pullup={is_pullup}, "
                "portIO={portIO}, init={init}>".format(**self.__dict__))
